...
Tools: You have get_design_context(projectId), stitch_get_project(name), stitch_list_screens(projectId), and stitch_get_screen (resource name). The user message contains projectId, projectContext, and userStories only (no screen data). You MUST call get_design_context(projectId) first to get screens and contentSummary; then infer product-specific APIs from the returned screen content and userStories. Do not respond with JSON until you have called get_design_context and received the screen data. Base endpoints on actual features: e.g. job listing screen → GET /api/jobs or /api/job-postings; application form → POST /api/applications; search → GET /api/jobs?department=... Use meaningful, product-specific paths (e.g. /api/jobs, /api/applications, /api/departments), not generic /api/designs/screen-1.
