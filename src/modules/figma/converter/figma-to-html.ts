...
export class FigmaToHTML {
  ...
  async convert(figmaNode: any) {
    ...
    if (this.options.useTailwind && this.tailwindConverter) {
      const conversionResult = await this.tailwindConverter.convertCSSToTailwind(css);

      // Replace generated classes with Tailwind classes in HTML
      finalHtml = this.replaceCSSClassesWithTailwind(html, conversionResult.tailwindClasses);
...
${finalHtml}
  ...
  async convertJSX(figmaNode: any, noTailwindImport = false) {
    ...
    const tJSX = transformJsx(`const ${generateComponentName(figmaNode.name)} = () => {
    return (
    ${jsx}
    );
  };
  ...
  convertNode(node: any, parentContext: { bounds?: any; node?: any } | null = null) {
    ...
    const classAttr = this.options.generateClasses ? ` class="${className}"` : '';

    switch (node.type) {
      case 'FRAME':
      case 'COMPONENT':
      case 'INSTANCE':
        return this.convertFrame(node, className, classAttr, inlineStyles, styles);
      case 'GROUP':
        return this.convertGroup(node, className, classAttr, inlineStyles, styles);
      case 'TEXT':
        return this.convertText(node, className, classAttr, inlineStyles, styles);
      case 'VECTOR':
      case 'RECTANGLE':
      case 'ELLIPSE':
      case 'STAR':
      case 'POLYGON':
      case 'LINE':
        return this.convertVector(node, className, classAttr, inlineStyles, styles);
      default:
        return this.convertGeneric(node, className, classAttr, inlineStyles, styles);
    }
  }
...
${childrenHTML}
  ...
  convertVector(
    ...
    if (this.shouldRenderAsSvg(node)) {
      ...
      const svgMarkup = this.generateVectorSVG(node, classAttr, svgInlineStyles);
      if (svgMarkup) {
  ...
  shouldRenderAsSvg(node: any) {
    ...
    const hasFillGeometry = Array.isArray(node.fillGeometry) && node.fillGeometry.length > 0;
    const hasStrokeGeometry = Array.isArray(node.strokeGeometry) && node.strokeGeometry.length > 0;
  ...
  generateVectorSVG(node: any, classAttr: string, inlineStyles: string) {
    ...
    const strokeResult = this.buildStrokeElements(node, strokePath, strokeRule, bounds);
    ...
    const viewBoxX = geometryBounds ? this.formatNumber(geometryBounds.minX) : '0';
    const viewBoxY = geometryBounds ? this.formatNumber(geometryBounds.minY) : '0';
    const viewBoxWidth = geometryBounds
      ? this.formatNumber(Math.max(0, geometryBounds.maxX - geometryBounds.minX))
      : width;
    const viewBoxHeight = geometryBounds
      ? this.formatNumber(Math.max(0, geometryBounds.maxY - geometryBounds.minY))
      : height;

    const content = shapes.map(line => `  ${line}`).join('\n');
    const containsImage = shapes.some(line => line.includes('<image '));
    const xlinkNamespace = containsImage ? ' xmlns:xlink="http://www.w3.org/1999/xlink"' : '';
    ...
    const updateFromPath = (path: string) => {
      ...
      while (index < tokens.length) {
        ...
        if (cmd === 'T') {
          const point = readPoint();
          if (!point) break;
          const absPoint = isRelative ? { x: current.x + point.x, y: current.y + point.y } : point;
        ...
        if (cmd === 'A') {
          ...
          const point = readPoint();
          if ([rx, ry, xAxisRotation, largeArc, sweep].some(v => v === null) || !point) {
            break;
          }
          const absPoint = isRelative ? { x: current.x + point.x, y: current.y + point.y } : point;
          const flags = [rx ?? 0, ry ?? 0, xAxisRotation ?? 0, largeArc ?? 0, sweep ?? 0];
  ...
  getWindingRule(geometry: any[] = []) {
    ...
    const rule = geometry.find(segment => segment && segment.windingRule)?.windingRule;
  ...
  buildFillElements(node: any, pathData: any, fillRule: any, bounds: any) {
    ...
    fills.forEach((fill, index) => {
      ...
      if (fill.type === 'SOLID') {
        const { color, opacity } = this.colorToSvgComponents(fill.color, fill.opacity);
        const opacityAttr = opacity < 1 ? ` fill-opacity="${this.formatNumber(opacity)}"` : '';
        elements.push(
          `<path d="${pathData}" fill="${color}" fill-rule="${fillRule}"${opacityAttr} />`
        );
      } else if (fill.type === 'GRADIENT_LINEAR') {
        const gradient = this.createLinearGradientDef(fill, node, bounds, `fill-${index}`);
        if (gradient) {
        ...
        const gradient = this.createRadialGradientDef(fill, node, bounds, `fill-${index}`);
        if (gradient) {
          defs.push(gradient.def);
          elements.push(
            `<path d="${pathData}" fill="url(#${gradient.id})" fill-rule="${fillRule}" />`
        ...
        const clipId = this.generateUniqueId(`clip-${this.sanitizeId(node.id)}-${index}-`);
        defs.push(
          `<clipPath id="${clipId}" clipPathUnits="userSpaceOnUse">\n      <path d="${pathData}" clip-rule="${clipRuleAttr}" />\n    </clipPath>`
        );

        const placeholderWidth = Math.max(1, Math.round(bounds?.width || 100));
        const placeholderHeight = Math.max(1, Math.round(bounds?.height || 100));
  ...
  createLinearGradientDef(paint: any, node: any, bounds: any, key: string) {
    ...
    const stops = paint.gradientStops.map((stop: any) => {
      const { color, opacity } = this.colorToSvgComponents(stop.color, paint.opacity);
      const offsetValue = this.formatNumber((stop.position ?? 0) * 100);
      const opacityAttr = opacity < 1 ? ` stop-opacity="${this.formatNumber(opacity)}"` : '';
  ...
  buildStrokeElements(node: any, pathData: any, strokeRule: any, bounds: any) {
    ...
    const strokeGeometry = Array.isArray(node.strokeGeometry)
      ? node.strokeGeometry.filter((segment: any) => segment && typeof segment.path === 'string')
      : [];
    ...
    const applyFillPaint = () => {
      ...
      if (strokePaint.type === 'GRADIENT_LINEAR') {
        const gradient = this.createLinearGradientDef(strokePaint, node, bounds, 'stroke');
        if (gradient) {
          defs.push(gradient.def);
      ...
      if (strokePaint.type === 'GRADIENT_RADIAL') {
        const gradient = this.createRadialGradientDef(strokePaint, node, bounds, 'stroke');
        if (gradient) {
          defs.push(gradient.def);
          return { paint: `url(#${gradient.id})`, opacity: 1 };
        }
    ...
    if (strokeGeometry.length > 0) {
      ...
      const fillOpacityAttr =
        paintResult.opacity < 1 ? ` fill-opacity="${this.formatNumber(paintResult.opacity)}"` : '';

      strokeGeometry.forEach((segment: any) => {
        const segmentRule = segment.windingRule ? segment.windingRule.toLowerCase() : geometryRule;
        elements.push(
    ...
    if (strokePaint.type === 'SOLID') {
      const { color, opacity } = this.colorToSvgComponents(strokePaint.color, strokePaint.opacity);
    ...
    const strokeWeight =
      typeof node.strokeWeight === 'number' && Number.isFinite(node.strokeWeight)
        ? node.strokeWeight
        : 1;
    ...
    if (dashArraySource && dashArraySource.length > 0) {
      const dashArray = dashArraySource.map((value: any) => this.formatNumber(value)).join(', ');
    ...
    const clipRuleAttr = strokeRule === 'evenodd' ? ` clip-rule="${strokeRule}"` : '';
    elements.push(`<path d="${pathData}" fill="none"${clipRuleAttr}${strokeAttrs} />`);
...
${childrenHTML}
  ...
  getTextStyles(style: any) {
    ...
    if (style.textDecoration) {
      textStyles.textDecoration = style.textDecoration.toLowerCase().replace('_', '-');
    }
    if (style.textCase) {
      if (style.textCase === 'UPPER') textStyles.textTransform = 'uppercase';
      else if (style.textCase === 'LOWER') textStyles.textTransform = 'lowercase';
      else if (style.textCase === 'TITLE') textStyles.textTransform = 'capitalize';
    }
    if (style.italic) {
  ...
  generateImageTag(node: any, styles: any) {
    ...
    const visibleFills = node.fills.filter((fill: any) => fill.visible !== false);
    if (!this.shouldUseImgTag(node, visibleFills)) return null;

    // Find the image fill
    const imageFill = visibleFills.find((fill: any) => fill.type === 'IMAGE' && fill.imageRef);
   ...
   */
  private replaceCSSClassesWithTailwind(html: string, classMapping: Map<string, string[]>): string {
    let result = html;
    ...
    for (const [generatedClass, tailwindClasses] of classMapping) {
      ...
      const escapedClass = generatedClass.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

      // Pattern 1: class="generatedClass" (entire class attribute)
      const singleClassRegex = new RegExp(`class="${escapedClass}"`, 'g');
      result = result.replace(singleClassRegex, `class="${tailwindClassString}"`);
