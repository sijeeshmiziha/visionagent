...
function getUniqueName(base: string, used: Set<string>, prog: NodePath<t.Program>): string {
  let candidate = base;
  let suffix = 2;
  while (
    used.has(candidate) ||
    prog.scope.hasBinding(candidate) ||
...
function createComponent(
  ...
  const additionalProps = Array.from(usedRefs).filter(ref => !sites.some(s => s.propKey === ref)); // Don't duplicate existing props

  const allPropKeys = [...sites.map(s => s.propKey), ...additionalProps];
...
function createInstance(
  ...
  additionalRefs.forEach(ref => {
    attrs.push(t.jsxAttribute(t.jsxIdentifier(ref), t.jsxExpressionContainer(t.identifier(ref))));
  });

  return t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier(compName), attrs, true), null, [], true);
}
...
export function transformJsx(
  ...
  traverse(ast, {
    JSXElement(path) {
      ...
      if (
        tagName &&
        (SKIP_AUTO_COMPONENT_TAGS.has(tagName.toLowerCase()) || generatedComponents.has(tagName))
      ) {
        return;
      }
      ...
      if (preferMap && areConsecutive(bestGroup.indices)) {
        ...
        const mapExpr = t.jsxExpressionContainer(
          t.callExpression(t.memberExpression(t.identifier(arrName), t.identifier('map')), [
              ...
              t.jsxElement(
                t.jsxOpeningElement(
                  ...
                  [
                    ...
                    t.jsxAttribute(
                      t.jsxIdentifier('key'),
                      t.jsxExpressionContainer(t.identifier('i'))
                    ),
                  ],
                  true
                ),
        ...
        const [start, end] = [
          bestGroup.indices[0]!,
          bestGroup.indices[bestGroup.indices.length - 1]!,
        ];
