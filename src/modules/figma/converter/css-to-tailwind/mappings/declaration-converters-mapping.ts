...
function convertBorderDeclarationValue(
  ...
  if (color) {
    ...
    classes = classes.concat(
      convertColorDeclarationValue(color, config.mapping.borderColor, classPrefix, 'color')
    );
  }
...
export function convertComposedSpacingDeclarationValue(
  ...
  (Object.keys(parsed) as ['top', 'right', 'bottom', 'left']).forEach(key => {
    ...
    if (value && valuesMapping && classPrefix) {
      classes = classes.concat(
        convertSizeDeclarationValue(value, valuesMapping, classPrefix, remInPx, true)
      );
    }
  });
...
export const DECLARATION_CONVERTERS_MAPPING: DeclarationConvertersMapping = {
  ...
  'backdrop-filter': (declaration, config) => {
    ...
    const mappings: Record<string, any> = {
      ...
      brightness:
        config.tailwindConfig.corePlugins.backdropBrightness && config.mapping.backdropBrightness,
      contrast:
        config.tailwindConfig.corePlugins.backdropContrast && config.mapping.backdropContrast,
      grayscale:
        config.tailwindConfig.corePlugins.backdropGrayscale && config.mapping.backdropGrayscale,
      'hue-rotate':
        config.tailwindConfig.corePlugins.backdropHueRotate && config.mapping.backdropHueRotate,
      ...
      saturate:
        config.tailwindConfig.corePlugins.backdropSaturate && config.mapping.backdropSaturate,
